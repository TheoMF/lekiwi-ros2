...
namespace lekiwi_controller
{
  ...
  CallbackReturn LeKiwiInterface::on_init(const hardware_interface::HardwareInfo &hardware_info)
  {
    ...
    use_serial_ = hardware_info.hardware_parameters.count("use_serial") ? (hardware_info.hardware_parameters.at("use_serial") == "true") : false;

    serial_port_ = hardware_info.hardware_parameters.count("serial_port") ? hardware_info.hardware_parameters.at("serial_port") : "/dev/ttyACM0";

    serial_baudrate_ = hardware_info.hardware_parameters.count("serial_baudrate") ? std::stoi(hardware_info.hardware_parameters.at("serial_baudrate")) : 1000000;

    std::string joint_offsets_file = hardware_info.hardware_parameters.count("joint_offsets_file") ? hardware_info.hardware_parameters.at("joint_offsets_file") : "";
  ...
  CallbackReturn LeKiwiInterface::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
  {
    ...
    if (use_serial_)
    {
      ...
      // Read current position from all servos and initialize commands to current positions
      for (size_t i = 0; i < info_.joints.size(); ++i)
      {
  ...
  hardware_interface::return_type LeKiwiInterface::write(const rclcpp::Time &time, const rclcpp::Duration &period)
  {
    ...
    if (first_call)
    {
      RCLCPP_INFO(rclcpp::get_logger("LeKiwiInterface"),
                  "write() first call - use_serial_=%d, torque_enabled_=%d, num_joints=%zu",
                  use_serial_, torque_enabled_, info_.joints.size());
      first_call = false;
    }
